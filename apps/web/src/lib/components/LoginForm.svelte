...
<form method="post" action= {action} class="max-w-lg w-full space-y-6 p-10	bg-primary rounded-lg shadow-md">
	...
	<label class="input input-bordered flex items-center gap-2">
		...
		<input type="text" class="grow" name="email" placeholder="Email"
					 autofocus value="{form?.email === undefined? '': form.email}" />
	</label>
	<label class="input input-bordered flex items-center gap-2">
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4 opacity-70">
	...
	<div class="w=full flex items-end">
		<a href="/register" class="text-secondary w-full text-right">Wachtwoord vergeten?</a>
	</div>
