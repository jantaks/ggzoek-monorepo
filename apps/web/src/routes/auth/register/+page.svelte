...
<div class="flex flex-col items-center justify-center grow h-screen">
	...
	<form method="post" class="max-w-lg w-full space-y-6 p-10	bg-primary rounded-lg shadow-md">
		<h1 class="font-bold text-3xl">Registreer gratis</h1>
		{#if form?.errors}
			{form.errors}
		{/if}
		...
		<label class="input input-bordered flex items-center gap-2">
			...
			<input type="password" class="grow" name="password" placeholder="Wachtwoord" />
		</label>
		<div class="flex flex-row justify-between bg-yellow-300">
			<div class="">
				<input type="checkbox" class="checkbox bg-base-100" />
			</div>
			<div class="px-6">
				<label for="terms" class="font-light text-gray-500 dark:text-gray-300">Ik ga akkoord met de algemene voorwaarden van GGZoek</label>
			</div>
		...
		<button type="submit" class="btn btn-secondary w-full text-white">Login</button>
		<p>Nog geen account? <a href="/register" class="text-secondary ">Registreer hier.</a></p>
