...
<div class="w-full">
  <Label.Root
    class="text-pink-100 text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
    for="distance-slider"
  ...
  >
    ...
    {#each thumbs as thumb}
      <Slider.Thumb
        {thumb}
        class="block size-[20px] cursor-pointer rounded-full border border-border-input bg-background shadow
          transition-colors hover:border-pink-500 focus-visible:outline-none focus-visible:ring-2
