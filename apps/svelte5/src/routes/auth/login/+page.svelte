...
<div class="flex flex-col items-center justify-center grow h-screen">
  <form action="/auth/login" class="max-w-lg w-full space-y-6 p-10	bg-pink-500 rounded-lg shadow-md" method="post">
    ...
    <label class="input input-bordered flex items-center gap-2">
      ...
      <input autofocus class="grow" name="email" placeholder="Email"
             type="text" value="{form?.email === undefined? '': form.email}" />
    </label>
    <label class="input input-bordered flex items-center gap-2">
      <svg class="w-4 h-4 opacity-70" fill="currentColor" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="w=full flex items-end">
      <a class="text-secondary w-full text-right" href="/register">Wachtwoord vergeten?</a>
    </div>
