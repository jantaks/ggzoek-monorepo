...
export async function correctSpelling() {
    ...
    for (const vacature of vacatures) {
        for (const field in vacature) {
            let isUpdated = false;
            let fieldSynonyms: Synonyms = getSynonyms(field);
...
function findSynonyms(value: string, synonyms: Synonyms) {
    ...
    for (const name of preferredSpelling) {
        ...
        const cleanedValue = value.normalize("NFD").replace(/[‘’'`\s\u0300-\u036f-]/g, '').toLowerCase();
        const cleanedSynonyms = synonyms[name].map(s => s.normalize("NFD").replace(/[‘’'`\s\u0300-\u036f-]/g, '').toLowerCase());
